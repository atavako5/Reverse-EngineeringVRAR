...
public class SpawnOnPlane : MonoBehaviour, IPointerClickHandler {
    ...
    public void OnPointerClick(PointerEventData eventData) {
        ...
        if (sessionOrigin.Raycast(ray, raycastHits, TrackableType.PlaneWithinInfinity)) {
            ...
            prefabToSpawn.transform.position = new Vector3(pose.position.x, pose.position.y, pose.position.z);
            prefabToSpawn.transform.rotation = pose.rotation;

            var forward = -mainCamera.transform.forward;
            forward.y = 0f;
            prefabToSpawn.transform.forward = forward.normalized;
